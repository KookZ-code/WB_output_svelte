...
/** Last N scans for a machine+package before shift start — for shift-boundary context. */
export function queryPrevTailRecords(
  w: ShiftWindow,
  machineId: string,
  packageKey: string,
  limit: number
  ...
  const rows = db()
    ...
    .all({ machine: machineId, pkg: packageKey, start: w.start, limit }) as Array<{
    created_at: string; lot_id: string; pkg: string;
    uph: number; bonded_unit: number; delta_bonded: number; badge_no: string;
  }>;
